...
struct flags_total { //estrutura que contém as flags das frequEncias e variáveis controlados pelo utilizador
	...
	float cresce_algo_div; //factor multiplicador da taxa de variação quando uma nota entra no algortimo
	float decresce_algo_div; //factor multiplicador da taxa de variação quando uma nota entra no algortimo
...
typedef struct _fbcontrolresist { //estrutura que define o objecto
...
static t_class* s_fbcontrolresist_class;

void* fbcontrolresist_new();
void fbcontrolresist_list(t_fbcontrolresist*, t_symbol*, long, t_atom*);

float note_accepted_deviation(int); //calcula o desvio aceitavel para reconhecer uma frequencia como outra
void quicksort_array_of_structures(t_fbcontrolresist*, int, int, int);//algoritmo de sorting. 0 se for do freqs_in, 1 se for do fcentral. Faz sort de array of pointers to structures
int check_partial(int, int, int);//compara duas frequênicas e um multiplicador, verificando se a hz2 é partial de hz1


void fbcontrolresist_init_flags(t_fbcontrolresist*); //função que inicia os valores da estrutura "flags" que determinam o comportamento do algorítmo
void fbcontrolresist_fill_freqs_in(t_fbcontrolresist*, int*, long, float, float, long); //adiciona ao freqs_in a informação da lista. Incrementa +1 à variável que conta o tamanho do freqs_in
int fbcontrolresist_in_alg(t_fbcontrolresist*, int, float); //para uma determinada nota do fcentral, verifica se esta deverá entrar para o algorítmo de controlo ou não
float fbcontrolresist_new_amp(t_fbcontrolresist*, int, float); //devolve a nova amplitude para uma determinada frequencia consoante o algortimo ativo
void fbcontrolresist_freqs_in_notes(t_fbcontrolresist*, int);//numa estrutura do tipo freqs (sorted), encontra as notas e harmónicos
void fbcontrolresist_update_fcentral(t_fbcontrolresist*, int, int*); //atualiza o fcentral, adicionado notas, organizando o pointer, eliminado residuos
void fbcontrolresist_dup_ddown_calc(t_fbcontrolresist*, int); //calcula os desvios para cada nota do fcentral
void fbcontrolresist_fcentral_inactive_note_delete(t_fbcontrolresist*, int*, int); //elimina notas inativas do fcentral
void fbcontrolresist_freqsin_addto_fcentral(t_fbcontrolresist*, int, int, int); //adiciona uma frequência do freqs_in ao fcentral
void fbcontrolresist_ampR_slope_calc(t_fbcontrolresist*, int); //calcula declives com base nas amplitudes em ampR
float fbcontrolresist_ampR_ampE_slope_calc(t_fbcontrolresist*, int, float); //calcula declives com base na ampE e na ampR
int fbcontrolresist_compare_freqs_in(t_fbcontrolresist*, int, float, float, long, float*); //compara a frequencia que está a ser analisada com os valores em fcentral, procurando a sua existência 
void fbcontrolresist_flags_calc(t_fbcontrolresist*, int); //calcula as flags de ampMax, ampMin, taxas de variação, etc
float fbcontrolresist_calc_taxa(t_fbcontrolresist*, float, int); //dá o valor de taxa de variação
...
void ext_main(void* r)
{
	...
	t_class* c;
	c = class_new("fbcontrolresist~", (method)fbcontrolresist_new, (method)NULL, sizeof(t_fbcontrolresist), 0L, 0);

	class_addmethod(c, (method)fbcontrolresist_list, "list", A_GIMME, 0);

	class_register(CLASS_BOX, c);

	s_fbcontrolresist_class = c;

}

void* fbcontrolresist_new()
{
	t_fbcontrolresist* x = (t_fbcontrolresist*)object_alloc(s_fbcontrolresist_class);
	...
	fbcontrolresist_init_flags(x);//inicializa as flags	

	return x;

}

void fbcontrolresist_list(t_fbcontrolresist* x, t_symbol* s, long argc, t_atom* argv)
{
	...
	if (proxy_getinlet((t_object*)x) == 0) //lista recebida no primeiro inlet
	{
		...
		if (x->globalFrame > -1)//estamos em análise ativa
		{
			...
			if (l_osc != 0)
			{
				...
				indice_note_fcentral = fbcontrolresist_compare_freqs_in(x, x->size_fcentral, l_hz, l_amp, l_flag, &l_amp);
				...
				if (indice_note_fcentral == -1) //não é nota, está a baixo do valor de amp min ou estamos no primeiro frame
				{
					...
					fbcontrolresist_fill_freqs_in(x, &(x->size_freqs_in), l_osc, l_hz, l_amp, l_flag); //preenche o freqs_in caso seja freq ativa

					outlet_list(x->m_outlet, NULL, 4, x->list_out);

				}
				else if (x->ptr_fcentral[indice_note_fcentral]->inalgo_cresce == 1 || x->ptr_fcentral[indice_note_fcentral]->inalgo_decresce == 1 || fbcontrolresist_in_alg(x, indice_note_fcentral, l_amp) == 1) //verifica se esta no algoritmo ou se deveria estar
				{

					l_amp_new = fbcontrolresist_new_amp(x, indice_note_fcentral, l_amp);

					atom_setfloat(x->list_out + 2, l_amp_new);
					fbcontrolresist_fill_freqs_in(x, &(x->size_freqs_in), l_osc, l_hz, l_amp, l_flag);
			...
			else
			{
				...
				fbcontrolresist_freqs_in_notes(x, x->size_freqs_in);
				fbcontrolresist_update_fcentral(x, x->size_freqs_in, &x->size_fcentral);
				fbcontrolresist_ampR_slope_calc(x, x->size_fcentral);
				fbcontrolresist_flags_calc(x, x->size_fcentral);
	...
	case 10:
		...
		if (marcador != 10) { post("Divisor de crescimento"); }
		x->flags.cresce_algo_div = in_valuef;
	...
	case 11:
		...
		if (marcador != 11) { post("Divisor de diminuicao"); }
		x->flags.decresce_algo_div = in_valuef;
...
void fbcontrolresist_init_flags(t_fbcontrolresist* x)
{
	...
	x->flags.taxacresce = 0.01;
	x->flags.taxadecresce = -0.01;

	x->flags.cresce_algo_div = 100;
	x->flags.decresce_algo_div = 100;
...
void fbcontrolresist_fill_freqs_in(t_fbcontrolresist* x, int* size_freqs_in, long osc, float hz, float amp, long flag) //adiciona ao freqs_in a informação da lista. Incrementa +1 à variável que conta o tamanho do freqs_in
{
...
int fbcontrolresist_compare_freqs_in(t_fbcontrolresist* x, int size_fcentral, float hz, float amp, long flag, float* ptr_l_amp) //compara a frequencia que está a ser analisada com os valores em fcentral, procurando a sua existência 
{
	...
	if (hz > x->flags.max_freq_to_algo) { return -1; } //se não estiver ligada a separação por notas ou a freq for maior que o valor máximo, não é considerada
	...
	for (i = 0; i < size_fcentral; i++)
	{
		...
		{
			if (abs(x->globalFrame - x->ptr_fcentral[i]->frame) > FRAMESTODELETE || (amp < x->flags.ampmin && x->ptr_fcentral[i]->inalgo_decresce == 0))
			{
...
int fbcontrolresist_in_alg(t_fbcontrolresist* x, int indice_fcentral, float amp_actual) //para uma determinada nota do fcentral, verifica se esta deverá entrar para o algorítmo de controlo ou não
{

	if (x->flags.algoOn == 1 && x->ptr_fcentral[indice_fcentral]->framesactive > x->flags.frames_to_active && x->size_fcentral > 1) //se o algorítmo estive ativo e a frequencia já for considerada ativa
	{

		if (x->ptr_fcentral[indice_fcentral]->slope > x->flags.taxacresce && (indice_fcentral == x->flags.freqMaxAmp || indice_fcentral == x->flags.freqMaxAmp2))
		{
		...
		else if (x->ptr_fcentral[indice_fcentral]->slope < x->flags.taxadecresce && (indice_fcentral == x->flags.freqMaxAmp || indice_fcentral == x->flags.freqMaxAmp2))
		{
...
float fbcontrolresist_new_amp(t_fbcontrolresist* x, int indice_note_fcentral, float amp) //devolve a nova amplitude para uma determinada frequencia consoante o algortimo ativo
{
	float nova_amp, nova_ampE;
	float nova_taxa, taxaE, nova_taxaE;

	if (x->ptr_fcentral[indice_note_fcentral]->inalgo_cresce == 1)
	{

		if (indice_note_fcentral != x->flags.freqMaxAmp && indice_note_fcentral != x->flags.freqMaxAmp2)
		{

			x->ptr_fcentral[indice_note_fcentral]->inalgo_cresce = 0;
			x->ptr_fcentral[indice_note_fcentral]->inalgo_decresce = 0;

			x->ptr_fcentral[indice_note_fcentral]->ampE = amp;

			return amp;

		}

		nova_taxa = x->ptr_fcentral[indice_note_fcentral]->slope / fbcontrolresist_calc_taxa(x, x->ptr_fcentral[indice_note_fcentral]->ampE, 1);
		taxaE = fbcontrolresist_ampR_ampE_slope_calc(x, indice_note_fcentral, amp);
		nova_amp = (expf(nova_taxa * 0.1)) * x->ptr_fcentral[indice_note_fcentral]->ampE;


		if (taxaE < 0) //taxa de variação num que estava a cresver, agora está a diminuir
		{
			nova_taxaE = taxaE / fbcontrolresist_calc_taxa(x, x->ptr_fcentral[indice_note_fcentral]->ampE, 0);
			nova_ampE = (expf(nova_taxaE * 0.1)) * x->ptr_fcentral[indice_note_fcentral]->ampE;

			x->ptr_fcentral[indice_note_fcentral]->inalgo_cresce = 0;
			x->ptr_fcentral[indice_note_fcentral]->inalgo_decresce = 1;

			x->ptr_fcentral[indice_note_fcentral]->ampE = nova_ampE;

			return nova_ampE;


		}
		else if (taxaE > 0) //ainda está a crescer
		{

			if (indice_note_fcentral == x->flags.freqMaxAmp2)
			{
				nova_taxaE = taxaE / (fbcontrolresist_calc_taxa(x, x->ptr_fcentral[indice_note_fcentral]->ampE, 1) * 3);
			}
			else
			{

				nova_taxaE = taxaE / fbcontrolresist_calc_taxa(x, x->ptr_fcentral[indice_note_fcentral]->ampE, 1);
	...
	else if (x->ptr_fcentral[indice_note_fcentral]->inalgo_decresce == 1)
	{
		...
		nova_taxa = x->ptr_fcentral[indice_note_fcentral]->slope / fbcontrolresist_calc_taxa(x, x->ptr_fcentral[indice_note_fcentral]->ampE, 0);
		taxaE = fbcontrolresist_ampR_ampE_slope_calc(x, indice_note_fcentral, amp);
		nova_amp = (expf(nova_taxa * 0.1)) * x->ptr_fcentral[indice_note_fcentral]->ampE;

		if (taxaE > 0) //taxa de variação num que estava a decrescer, agora está a diminuir
		{
		...
		else if (taxaE < 0) //ainda está a dimnuir
		{

			if (indice_note_fcentral == x->flags.freqMaxAmp) 
			{ 
				nova_taxaE = taxaE / (fbcontrolresist_calc_taxa(x, x->ptr_fcentral[indice_note_fcentral]->ampE, 0)*3); 
			}
			else
			{
...
void quicksort_array_of_structures(t_fbcontrolresist* x, int first, int last, int freqs_in_or_fcentral) //algoritmo de sorting. 0 se for do freqs_in, 1 se for do fcentral. Faz sort de array of pointers to structures
{
...
void fbcontrolresist_freqs_in_notes(t_fbcontrolresist* x, int size)//numa estrutura do tipo freqs (sorted), encontra as notas e harmónicos
{
...
void fbcontrolresist_dup_ddown_calc(t_fbcontrolresist* x, int fcentral_size) //calcula os desvios para cada nota do fcentral
{
...
void fbcontrolresist_fcentral_inactive_note_delete(t_fbcontrolresist* x, int* fcentral_size, int freqsin_size) //elimina notas do fim do fcentral que já estejam inativas
{
	int i, k;
...
void fbcontrolresist_freqsin_addto_fcentral(t_fbcontrolresist* x, int indice_freqsin, int indice_fcentral, int subs_or_add_or_end) //adiciona uma frequência do freqs_in ao fcentral
{
...
void fbcontrolresist_update_fcentral(t_fbcontrolresist* x, int freqsin_size, int* fcentral_size) //atualiza o fcentral, adicionado notas, organizando o pointer, eliminado residuos
{
	...
	if (x->fcentral[0].hz == -1) //verificar se é a primeira vez que chamamos esta função (fcentral vazio)
	{
		...
		*fcentral_size = k;
		...
		fbcontrolresist_dup_ddown_calc(x, *fcentral_size);
	...
	else //fcentral já tem frequências
	{
					...
					if (x->ptr_freqs_in[i]->freq < x->ptr_fcentral[k]->ddown)//ultrapssou o limite
					{
						...
						if (hz_anterior != -1)
						{
							...
							else
							{
								...
								fbcontrolresist_freqsin_addto_fcentral(x, i, *fcentral_size + after_fcentral, 2);
								after_fcentral += 1;
					...
					else //estou dentro do intervalo desta freq do fcentral
					{
						...
						if (hz_anterior != -1)
						{
							...
							else
							{
								...
								fbcontrolresist_freqsin_addto_fcentral(x, i, k, 1);
								break;
		...
		*fcentral_size += after_fcentral;
		...
		fbcontrolresist_fcentral_inactive_note_delete(x, fcentral_size, freqsin_size);
...
void fbcontrolresist_ampR_slope_calc(t_fbcontrolresist* x, int size_fcentral) //calcula declives com base nas amplitudes em ampR
{
...
float fbcontrolresist_ampR_ampE_slope_calc(t_fbcontrolresist* x, int indice, float amp_atual)
{
	float zy; //somatÛrio de tempo*amp
	int z; //sumatÛrio de tempo
	float y; //sumatÛrio de amp
	...
	zy = logf(x->ptr_fcentral[indice]->ampE) + 2 * logf(amp_atual);
	z = 3;
	y = logf(x->ptr_fcentral[indice]->ampE) + logf(amp_atual);
	zz = 5;

	s = ((2 * zy - z * y) / (2 * zz - z * z)) / 0.1;

	return s;

}

void fbcontrolresist_flags_calc(t_fbcontrolresist* x, int size_fcentral) //calcula as flags de ampMax, ampMin, taxas de variação, etc
{
	...
	if (ampMaxI == ampMaxI2) { ampMaxI2 = -1; }
...
float fbcontrolresist_calc_taxa(t_fbcontrolresist* x, float amp, int cresce_descresce)
{

	if (cresce_descresce == 0)//esta a diminuir
	{

		return x->flags.decresce_algo_div;

	}
	else
	{

		return x->flags.cresce_algo_div;
